...
- [ ] **Exit criterion:** approved data model and API/TUI contracts; explicit migration strategy from legacy `task` sessions; no implementation yet depends on unverified external-plugin behavior.
...
- [ ] Add versioned configuration schemas for global shared and global role model/context defaults, named teams, default team selection, per-role provider/model/variant/context limit, prompts, permissions, concurrency, and workspace policy.
