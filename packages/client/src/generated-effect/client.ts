...
type Endpoint3_3Request = Parameters<RawClient["server.session"]["session.usage"]>[0]
type Endpoint3_3Input = { readonly since?: Endpoint3_3Request["query"]["since"] }
const Endpoint3_3 = (raw: RawClient["server.session"]) => (input?: Endpoint3_3Input) =>
  raw["session.usage"]({ query: { since: input?.["since"] } }).pipe(
    Effect.mapError(mapClientError),
    Effect.map((value) => value.data),
  )
